...
/** @ignore */
const reHex = /^0[xX][0-9a-fA-F]+$/;

/** @ignore */
const reBin = /^0[bB][01]+$/;

/** @ignore */
const reOctal = /^0[oO][0-7]+$/;

/** @ignore */
const reExp = /^[-+]?[0-9]*\.?[0-9]+([eE][-+]?[0-9]+)$/;
...
export class Numberifier {
  ...
  number(src: Primitive): number | null | undefined {
    ...
    function evalString(src: string): number | null | undefined {
      src = src.trim();
      if (reHex.test(src) && THIS.ignoreHex) return NAN_VALUE;
      if (reBin.test(src) && THIS.ignoreBin) return NAN_VALUE;
      if (reOctal.test(src) && THIS.ignoreOctal) return NAN_VALUE;
      ...
      if (reExp.test(src) && THIS.ignoreExp) {
        return NAN_VALUE;
      }
      return Number(src);
    }
 ...
 *
 */
export const NaN: number = Number.NaN; // tslint:disable-line:variable-name
